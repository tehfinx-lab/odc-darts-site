  ...
  for (const row of rows.slice(1)) {
    ...
    if (week === MVP_WEEK) {
      for (const candidate of [
        ...
        // Skip forfeits / walkovers: legs get recorded but no darts were
        // actually thrown, so there is no 3-dart average. A real win always
        // has an average above zero.
        if (!(Number(stats.avg) > 0)) continue;
