...
export default function App() {
  // Read the tab straight out of the URL hash so refreshing, sharing a link,
  // and the browser back/forward buttons all land on the right page.
  ...
  // wrapper: changing tab also updates the URL (no page reload)
  const setActive = (id) => {
    setActiveState(id);
    if (typeof window !== "undefined") {
      const target = `#${id}`;
      if (window.location.hash !== target) {
        window.history.pushState(null, "", id === "home" ? window.location.pathname : target);
      }
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
  };

  // on first load, honour whatever is in the URL
  useEffect(() => {
    setActiveState(pageFromHash());
  }, []);

  // back / forward buttons
  useEffect(() => {
    const onPop = () => setActiveState(pageFromHash());
