    ...
    <>
      <header className="sticky top-0 z-50 border-b border-odcCream/10 bg-black/75 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          <button onClick={() => setOpen(true)} className="rounded-2xl border border-odcCream/15 p-3 xl:hidden"><Menu size={22} /></button>
        </div>
      </header>
...
function PlayerDetailsModal({ player, masterStats, matches, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/75 p-4 backdrop-blur">
      <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-3xl border border-odcCream/15 bg-odcBlack p-5 shadow-glow">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
    ...
    <section className="mx-auto max-w-7xl px-4 py-10">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        ...
        <div className="relative mb-7">
          <select value={selected} onChange={(e) => setSelected(e.target.value)} className="w-full appearance-none rounded-2xl border border-odcCream/15 bg-odcNavy px-5 py-4 pr-12 font-black text-odcCream outline-none md:w-72">
      ...
      {rows.length === 0 ? (
        <Card><p className="text-lg font-black">No results available for this division yet.</p></Card>
      ) : (
    ...
    <section className="mx-auto max-w-7xl px-4 py-10">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <SectionTitle
          ...
          text="Explore player performance cards, season records and recent match history across the ODC league."
        />
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10">
      <SectionTitle
        ...
        title="Weekly MVPs"
        text="Recognising the top performer from each division based on winning displays and match statistics."
      />
      ...
      {mvps.length === 0 ? (
        <Card>
          ...
          <p className="mt-2 text-odcCream/60">
            Weekly MVPs will appear here once the relevant ODC match results have been recorded.
          </p>
        </Card>
      ) : (
...
function EventsPage() {
  return <section className="mx-auto max-w-7xl px-4 py-10"><SectionTitle kicker="Events" title="ODC Events" text="Tournament results, special events and community competitions will be shown here." /><Card className="max-w-2xl"><h3 className="text-2xl font-black">Events coming soon</h3><p className="mt-3 text-odcCream/65">This section will be used for ODC tournament winners, event results and community competition updates.</p><div className="mt-5"><SocialButtons /></div></Card></section>;
}
...
export default function App() {
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top,#102131_0%,#050505_42%,#000_100%)] pb-24 text-odcCream xl:pb-0"><Header active={active} setActive={setActive} />{status === "fallback" && <div className="bg-odcRed px-4 py-2 text-center text-sm font-black text-white">Google Sheets data could not be loaded. Showing demo data.</div>}<motion.div key={active} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}>{active === "home" && <HomePage setActive={setActive} data={data} status={status} onSelectMatch={setSelectedMatch} />}{active === "fixtures" && <FixturesPage data={data} />}
        {active === "results" && <ResultsPage data={data} onSelectMatch={setSelectedMatch} />}{active === "tables" && <TablesPage data={data} />}{active === "players" && <PlayersPage data={data} onSelectPlayer={setSelectedPlayer} />}{active === "leaderboards" && <LeaderboardsPage data={data} />}{active === "mvps" && <MvpsPage data={data} />}{active === "events" && <EventsPage />}</motion.div><footer className="border-t border-odcCream/10 px-4 py-8 text-center text-sm text-odcCream/45">© ODC — Online Darts Circuit. Competitive online darts, organised properly.</footer><BottomNav active={active} setActive={setActive} /><MatchDetailsModal match={selectedMatch} onClose={() => setSelectedMatch(null)} />
      <PlayerDetailsModal player={selectedPlayer} masterStats={data.masterStats || {}} matches={data.allResults || []} onClose={() => setSelectedPlayer(null)} /></main>;
}
