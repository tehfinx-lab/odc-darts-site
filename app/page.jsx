...
function Card({ children, className = "" }) {
  return (
    <div
      className={`rounded-3xl border border-odcCream/10 bg-white/[0.055] p-5 shadow-cream backdrop-blur ${className}`}
    >
      {children}
    </div>
  );
...
function SectionTitle({ kicker, title, text }) {
  return (
    <div className="mb-7">
      <p className="text-xs font-black uppercase tracking-[0.35em] text-odcRed">
        {kicker}
      </p>
      ...
      {text && (
        <p className="mt-3 max-w-2xl text-sm leading-7 text-odcCream/65 md:text-base">
          {text}
        </p>
      )}
    </div>
  ...
  return (
    <Card className="p-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-odcCream/45">
            {label}
          </p>
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <button onClick={() => go("home")} className="flex items-center gap-3 text-left">
            <img
              src="/odc-logo.jpg"
              alt="ODC logo"
              className="h-11 w-11 rounded-full object-cover ring-2 ring-odcCream/20"
            />
            <div>
          ...
          <button
            ...
            className="rounded-2xl border border-odcCream/15 p-3 md:hidden"
          >
            <Menu size={22} />
          </button>
        </div>
            ...
            <div className="mb-8 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-odcCream/50">Jump around the league hub</p>
                </div>
              </div>
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/75 p-4 backdrop-blur">
      ...
      >
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.3em] text-odcRed">
              {match.division}
            </p>
        ...
        <div className="mb-5 grid grid-cols-[1fr_auto_1fr] items-center gap-3 rounded-3xl bg-odcNavy p-5">
          ...
          <p className="rounded-2xl bg-odcRed px-5 py-2 text-2xl font-black text-white">
            {match.score}
          </p>
    ...
    <div>
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 px-4 py-10 md:grid-cols-[1.05fr_0.95fr] md:py-20">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-odcRed/35 bg-odcRed/10 px-4 py-2 text-xs font-black uppercase tracking-[0.22em] text-odcCream/85">
              <Target size={15} /> {status === "live" ? "Live Google Sheets data" : "Loading league data"}
            </div>

            <h1 className="text-4xl font-black tracking-tight md:text-7xl">
              Online Darts, <span className="text-odcRed">Done Properly.</span>
            </h1>

            <p className="mt-5 max-w-2xl text-base leading-8 text-odcCream/70 md:text-lg">
              Welcome to ODC — fixtures, tables, player stats, rankings and match breakdowns in one clean league hub.
            </p>

            <div className="mt-7 flex flex-wrap gap-3">
              <button
                onClick={() => setActive("tables")}
                className="rounded-2xl bg-odcRed px-5 py-3 font-black text-white shadow-glow"
              >
                View Tables
              </button>
              ...
              <button
                ...
                className="rounded-2xl border border-odcCream/20 px-5 py-3 font-black"
              >
                Player Stats
              </button>
            </div>
          ...
          >
            <Card className="relative overflow-hidden p-6">
              <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(229,29,42,.22),transparent_58%)]" />
              <img
                src="/odc-logo.jpg"
                alt="ODC logo"
                className="relative mx-auto aspect-square w-52 rounded-full object-cover shadow-glow ring-4 ring-odcCream/15 md:w-72"
              />
            </Card>
          </motion.div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-8">
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <SmallStat label="Recent Results" value={data.results.length} icon={CalendarDays} />
      ...
      <section className="mx-auto max-w-7xl px-4 py-10">
        <SectionTitle
          ...
          text="Click any result to view all stats from that match row."
        />

        <div className="grid gap-4 md:grid-cols-3">
          {data.results.slice(0, 6).map((r, index) => (
            <button
              key={r.id || `${r.home}-${r.away}-${index}`}
            ...
            >
              <Card className="h-full transition hover:-translate-y-1 hover:border-odcRed/40">
                ...
                <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                  ...
                  <p className="rounded-xl bg-odcRed px-3 py-1 text-lg font-black text-white">
                    {r.score}
                  </p>
    ...
    <section className="mx-auto max-w-7xl px-4 py-10">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <SectionTitle
          kicker="Tables"
          title="League Tables"
          text="Divisions are created automatically from the Matches sheet."
        />
    ...
    <section className="mx-auto max-w-7xl px-4 py-10">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <SectionTitle
          kicker="Players"
          title="Player Stats"
          text="Built automatically from both players in every match row."
        />
...
function LeaderboardsPage({ data }) {
  const boards = useMemo(
    () => [
      ...
      [
        "Highest Checkout",
        [...data.players].sort((a, b) => b.highCheckout - a.highCheckout),
        "highCheckout",
        Medal,
      ],
      [
        "Best Leg",
        [...data.players]
          .filter((p) => Number(p.bestLeg) > 0)
          .sort((a, b) => a.bestLeg - b.bestLeg),
        "bestLeg",
        Zap,
      ],
    ],
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10">
      <SectionTitle
        kicker="Rankings"
        title="Leaderboards"
        text="Automatically calculated from every match in your Matches sheet."
      />
        ...
        {boards.map(([title, list, key, Icon]) => (
          <Card key={title}>
            ...
            {list
              ...
              .map((p, i) => (
                <div
                  key={`${title}-${p.name}-${p.division}`}
                  className="flex items-center justify-between border-t border-odcCream/10 py-3"
                >
                  <span className="font-bold">
                    {i + 1}. {p.name}
                  </span>
                  <span className="rounded-lg bg-odcCream/10 px-3 py-1 font-black">
                    {p[key]}
                  </span>
                </div>
              ))}
...
function EventsPage() {
  return (
    <section className="mx-auto max-w-7xl px-4 py-10">
      <SectionTitle
        kicker="Events"
        title="Nightly Tournament Winners"
        text="This section is ready. Add an Events sheet later and we can connect it properly."
      />

      <Card className="max-w-2xl">
        <h3 className="text-2xl font-black">Events sheet not connected yet</h3>
        <p className="mt-3 text-odcCream/65">
          The old fake/latest-match winner has been removed so this page does not show incorrect tournament data.
        </p>
...
export default function App() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#102131_0%,#050505_42%,#000_100%)] pb-24 text-odcCream md:pb-0">
      ...
      >
        ...
        {active === "players" && <PlayersPage data={data} />}
      ...
      <footer className="border-t border-odcCream/10 px-4 py-8 text-center text-sm text-odcCream/45">
        © ODC — Online Darts Circuit. Built for live league stats.
      </footer>
